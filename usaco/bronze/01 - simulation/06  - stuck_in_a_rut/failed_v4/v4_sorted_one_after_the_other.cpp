...
void debug(container &genericSequence, string id = "None", int depth = 0);

// debug utils
#ifdef DEBUG
template <typename container>
void debug(container &genericSequence, string id, int depth) {
  cout << "=============Debug (" << id << ") START =============\n";
  string prefix = "";
...
bool byCoord(array<ll, 6> mapa, array<ll, 6> mapb) {
  ...
  return mapa[POS_X] == mapb[POS_X] ? (mapa[POS_Y] < mapb[POS_Y])
                                    : (mapa[POS_X] < mapb[POS_X]);
}
...
void solve() {
  ...
  for (size_t i = 0; i < n; i++)
  {
    cout<<map[i][ORDER]<<"\t-\t"<<(char)map[i][ORIENTATION]<<"\t-\t"<<map[i][POS_X]<<"\t-\t"<<map[i][POS_Y]<<"\n";
  }
  ...
  for (size_t i = init; i < n; i++) {
     ...
     * A====			  |		A==== 	|A====
     */
    if (map[i][ORIENTATION] == 'E' && i + 1 < n &&
        map[i + 1][ORIENTATION] == 'E' && map[i][POS_Y] < map[i + 1][POS_Y]
     ...
     * A========    B=========
     */
    else if (map[i][ORIENTATION] == 'E' && i + 1 < n &&
             map[i + 1][ORIENTATION] == 'E' &&
             map[i][POS_Y] == map[i + 1][POS_Y]) {
      cout<<"\t\t2\n";
      map[i][IS_LOCKED] = true;
      map[i][GRASS_EATEN] = abs(map[i + 1][POS_X] - map[i][POS_X]);
      init += 1;
    }
    /**CHECKED+TEST
     *					|
     ...
     * 	A	  B
     */
    else if (map[i][ORIENTATION] == 'N' && i + 1 < n &&
             map[i + 1][ORIENTATION] == 'N' &&
             map[i][POS_X] < map[i + 1][POS_X]) {
      cout<<"\t\t\t\t\t5\n";
      map[i][IS_LOCKED] = true;
      map[i][GRASS_EATEN] = -1;
      init += 1;
